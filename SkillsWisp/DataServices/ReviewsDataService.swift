...
final class ReviewsDataService {
    ...
    func addReviewInDB(standard_id: String, subject_id: String, note_id: String, review: String) async throws {
        ...
        try await reviewsCollection.document(note_id).collection("user_id").document(user.uid).setData(userID, merge: false)
        let reviewDocRef = reviewsCollection.document(note_id).collection("user_id").document(user.uid).collection("user_reviews")
    ...
    func fetchReviewFromDB(standard_id: String, subject_id: String, note_id: String,
                           completion: @escaping ([ReviewModel]?) -> ()) async throws {
        
        let reviewDocRef = reviewsCollection.document(note_id).collection("user_id")
